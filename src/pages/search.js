...
const Search = ({ products, attributes }) => {
  ...
  const localizationSettings = settingItem.find(
    (value) => value.name === "localizationSetting"
  );
  ...
  const handleProductSorting = (data) => {
    ...
    const updatedQuery = { ...currentQuery, sort: data };
  ...
  const handleProductMoreLoading = () => {
    ...
    const currentQuery = router.query;
    // update the "newParam" query parameter
    const updatedQuery = { ...currentQuery, limit: visibleProduct + 10 };
    router.push({ query: updatedQuery });
  ...
  // const { setSortedField, productData } = useFilter(product_data);
              ...
              <div className="w-full">
                ...
                {product_data?.length === 0 ? (
                  ...
                  <div className="flex justify-between my-3 bg-orange-100 border border-gray-100 rounded p-3">
                    <h6 className="text-sm font-serif">
                      {t("common:totalI")}{" "}
                      <span className="font-bold">{product_data?.length}</span>{" "}
                      {t("common:itemsFound")}
                    </h6>
                ...
                {isLoading ? (
                  ...
                  <>
                    ...
                    {product_data?.length > visibleProduct && (
                      <button
...
export const getServerSideProps = async (context) => {
  const { title, _id, sort = "", limit } = context.query;
