...
const Coupon = ({ couponInHome }) => {
  ...
  const { storeCustomizationSetting, error } = useGetSetting();
  ...
  useEffect(() => {
    if (couponInHome) {
      const filteredCoupon = data?.filter(({ couponCode }) =>
        storeCustomizationSetting?.home?.discount_coupon_code.includes(
          couponCode
        )
      );
      setEnterCouponCodeList(filteredCoupon);
    } else {
      const filteredCoupon = data?.filter(({ couponCode }) =>
  ...
  //console.log('coupon-LocalizationSetting', LocalizationSetting)

  // console.log('coupon', enterCouponCodeList);
